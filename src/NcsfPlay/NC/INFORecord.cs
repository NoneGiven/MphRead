...
public class INFORecord<T> where T : INFOEntry, new()
{
	[DebuggerBrowsable(DebuggerBrowsableState.Never)]
	string DebuggerDisplay => $"INFO Record ({$"{typeof(T)}"[23..]}) - # of Entries: {this.entries.Count}";

	readonly List<(uint Offset, T? Entry)> entries = [];

	/// <summary>
	/// The entries contained within this record.
	/// </summary>
	public ReadOnlySpan<(uint Offset, T? Entry)> Entries => this.entries.AsSpan();

	/// <summary>
	/// The full (header + entries) size of this record.
	/// </summary>
	/// <remarks>
	/// <see cref="HeaderSize" /> + <see cref="SizeOfEntries" />.
	/// </remarks>
	public uint Size => this.HeaderSize + this.SizeOfEntries;
	...
	/// <summary>
	/// The size of just the entries of this record.
	/// </summary>
	public uint SizeOfEntries => (uint)this.entries.Sum(static e => e.Entry?.Size ?? 0U);
	...
	/// <param name="offset">
	/// The offset from the parent <see cref="INFOSection" />,
	/// needed because <paramref name="span" /> is passed in from the start of the record instead.
	/// </param>
	public void Read(ReadOnlySpan<byte> span, uint offset)
	{
		...
		foreach (uint entryOffset in entryOffsets)
			this.entries.Add((entryOffset, entryOffset == 0 ? null : new T().Read(span[(int)(entryOffset - offset)..]) as T));
	}
	...
	/// The offset from the parent <see cref="INFOSection" /> where the entries of this record should start from.
	/// </param>
	...
	public void WriteData(Span<byte> span)
	{
		...
		foreach (var (_, Entry) in this.entries)
			...
			{
				Entry.Write(span[(int)pos..]);
				pos += Entry.Size;
			}
	}

	/// <summary>
	/// Appends two INFO Records to each other.
	/// </summary>
	/// <remarks>
	/// This will not make any changes to the offsets of these records, that is expected to be corrected in a later step.
	/// </remarks>
	/// <param name="infoRecord1">The first <see cref="INFORecord{T}" />.</param>
	/// <param name="infoRecord2">The second <see cref="INFORecord{T}" />.</param>
	/// <returns>The combined <see cref="SYMBRecord" />.</returns>
	public static INFORecord<T> operator +(INFORecord<T> infoRecord1, INFORecord<T> infoRecord2)
	{
		INFORecord<T> newINFORecord = new();

		newINFORecord.entries.AddRange(infoRecord1.entries);
		newINFORecord.entries.AddRange(infoRecord2.entries);
