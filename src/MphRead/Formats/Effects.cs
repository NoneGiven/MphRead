...
namespace MphRead.Formats
{
    public static class Effects
    {
        ...
        private static void FxFunc14(IReadOnlyList<int> param, int a2, ref Vector3 vec, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            ...
            float value = Fixed.ToFloat(InvokeIntFunc(funcs[(uint)param[1]], a2, funcs));
        ...
        {
            float value1 = Fixed.ToFloat(InvokeIntFunc(funcs[(uint)param[0]], a2, funcs));
            float value2 = Fixed.ToFloat(InvokeIntFunc(funcs[(uint)param[1]], a2, funcs));
        ...
        {
            Vector3 temp = Vector3.Zero;
            float value = Fixed.ToFloat(InvokeIntFunc(funcs[(uint)param[0]], a2, funcs));
        ...
        private static int FxFunc22(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo: return cur_effect_elem->field_74;
            return 0;
        }

        private static int FxFunc24(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo: return cur_effect_elem->alpha;
            return 0;
        }

        private static int FxFunc25(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo: return cur_effect_elem->red;
            return 0;
        }

        private static int FxFunc26(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo: return cur_effect_elem->green;
            return 0;
        }

        private static int FxFunc29(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo: return cur_effect_elem->blue;
            return 0;
        }

        private static int FxFunc31(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo: return cur_effect_elem->field_44;
            return 0;
        }

        private static int FxFunc32(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo: return cur_effect_elem->field_48;
            return 0;
        }

        private static int FxFunc35(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo: return cur_effect_elem->field_54;
            return 0;
        }

        private static int FxFunc40(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo: eff_div_something
            float effDivSomething = Fixed.ToFloat(0x2000);
            if (a2 / effDivSomething <= Fixed.ToFloat(param[0]))
            {
                return param[1];
            }
            return param[2];
        ...
        private static int FxFunc41(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            // todo
            return 0;
        }

        private static int FxFunc42(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            return param[0];
        }

        private static int FxFunc43(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            return (int)Test.GetRandomInt1(4096);
        }

        private static int FxFunc45(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            return (int)Test.GetRandomInt1(0x168000);
        }

        private static int FxFunc46(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            return InvokeIntFunc(funcs[(uint)param[0]], a2, funcs) + InvokeIntFunc(funcs[(uint)param[1]], a2, funcs);
        }

        private static int FxFunc47(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            return InvokeIntFunc(funcs[(uint)param[0]], a2, funcs) - InvokeIntFunc(funcs[(uint)param[1]], a2, funcs);
        }

        private static int FxFunc48(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            return InvokeIntFunc(funcs[(uint)param[0]], a2, funcs) * InvokeIntFunc(funcs[(uint)param[1]], a2, funcs);
        }

        private static int FxFunc49(IReadOnlyList<int> param, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
            if (InvokeIntFunc(funcs[(uint)param[0]], a2, funcs) >= InvokeIntFunc(funcs[(uint)param[1]], a2, funcs))
            {
                return InvokeIntFunc(funcs[(uint)param[2]], a2, funcs);
            }
            return InvokeIntFunc(funcs[(uint)param[3]], a2, funcs);
        }
        ...
        public static int InvokeIntFunc(FxFuncInfo info, int a2, IReadOnlyDictionary<uint, FxFuncInfo> funcs)
        {
