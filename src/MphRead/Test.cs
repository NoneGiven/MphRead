...
namespace MphRead
{
    public static class Test
    {
        ...
        {
            IReadOnlyList<WeaponInfo> results = ParseWeaponInfo();
            foreach (WeaponInfo info in results)
            {
                Console.WriteLine($"{info.Name}:");
                Console.WriteLine($"{info.Field2C} / {info.Field2E} / {info.Field30}");
                Console.WriteLine($"{info.Field32} / {info.Field34} / {info.Field36}");
                Console.WriteLine();
            }
            Nop();
        }

        public static IReadOnlyList<WeaponInfo> ParseWeaponInfo()
        {
            int count = 18;
            int size = Marshal.SizeOf<WeaponInfo>();
            var results = new List<WeaponInfo>();
            var bytes = new ReadOnlySpan<byte>(new byte[]
            {
